...
export default function HomeHero() {
  const slides = SERVICES.map((s) => (
    <div key={s.title} className="hero-slide flex flex-col justify-center px-8 pb-6 pt-3 sm:px-12">
      <h3 className="font-display text-f6 leading-none text-ink-100">{s.title}</h3>
    ...
    <section className="hero-fill flex w-full flex-col bg-gradient-to-b from-navy via-navy to-blue-muted/50 pb-4 pt-0">
      <Container className="flex flex-1 flex-col">
        ...
        <div className="hero-row mt-8 grid grid-cols-1 gap-6 sm:shrink-0 sm:grid-cols-[minmax(0,1.5fr)_minmax(0,0.5fr)]">
          <div className="flex items-center overflow-hidden rounded-2xl bg-navy-soft py-2 [container-type:inline-size]">

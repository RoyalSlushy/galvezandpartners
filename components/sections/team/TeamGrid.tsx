...
import { TEAM, TEAM_HEADING } from "@/content/team";
...
export default function TeamGrid() {
  return (
    <section className="w-full bg-navy py-20 sm:py-28">
      <Container>
        <RevealOnScroll>
          <h1 className="font-heading text-f3 leading-none text-white">{TEAM_HEADING}</h1>
        </RevealOnScroll>

        <div className="mt-14 grid grid-cols-2 gap-8 sm:grid-cols-3 lg:grid-cols-4">
          {TEAM.map((m, i) => (
            <RevealOnScroll key={m.name} delay={0.05 * (i % 4)}>
              <figure className="text-center">
                <div className="overflow-hidden rounded-2xl bg-navy-soft">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={wixImage(m.photo, 400, 480)}
                    alt={m.name}
